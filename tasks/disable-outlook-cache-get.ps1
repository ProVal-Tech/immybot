...
'@ | Invoke-Expression
    ...
    function Get-OutlookSharedMailboxCache {
        [CmdletBinding()]
        [OutputType([string])]
        param ()
    
        $hives = Open-UserHives
        $returnObjects = @(foreach ($hive in $hives) {
                ...
                foreach ($officeVersion in $officeVersions) {
                    ...
                    foreach ($officeKey in $targetKeys) {
                        ...
                        Get-ItemProperty -Path $fullTargetPath -Name CacheOthersMail -ea 0
                        Get-ItemProperty -Path $fullTargetPath -Name DownloadSharedFolders -ea 0
                        $hive.Dismount()
                    }
                }
            }
        )
        return $returnObjects | ConvertTo-Json
    }
    return Get-OutlookSharedMailboxCache
}