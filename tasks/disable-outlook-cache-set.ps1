...
'@ | Invoke-Expression
    ...
    function Disable-OutlookSharedMailboxCache {
            ...
            foreach ($officeVersion in $officeVersions) {
                ...
                foreach ($officeKey in $targetKeys) {
                    ...
                    if (!(Test-Path -Path $fullTargetPath)) { 
                        New-Item -Path $fullTargetPath -Force | Out-Null
                    }
                    New-ItemProperty -Path $fullTargetPath -Name CacheOthersMail -Value 0 -Force | Out-Null
                    New-ItemProperty -Path $fullTargetPath -Name DownloadSharedFolders -Value 0 -Force | Out-Null
                }
            }
            $hive.Dismount()
        }
    }
    Disable-OutlookSharedMailboxCache
}